...
export const useStyles = createUseStyles(
  ({ palette, borderRadius, typography }) => ({
    segmentedButton: {
      ...
      '&:not(:first-of-type)': {
        borderLeftWidth: ({ btnsDirection }: Partial<SegmentedButtonProps>) =>
          btnsDirection === 'row' ? 0 : 1,
        borderTopWidth: ({ btnsDirection }: Partial<SegmentedButtonProps>) =>
          btnsDirection === 'row' ? 1 : 0
      },
